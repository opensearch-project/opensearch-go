...
# Sub-modules are auto-discovered: every go.mod except the root one. New nested
# modules (cmd/*, osprom, osotel, ...) are picked up by test-unit and lint with
# no Makefile change. Paths are sorted for deterministic ordering.
SUBMODULES := $(shell find . -name go.mod -not -path './go.mod' -not -path '*/.*' -exec dirname {} \; | sed 's|^\./||' | sort)
...
ifdef race
	...
	@for mod in $(SUBMODULES); do \
		tag=$$(echo "$$mod" | tr '/' '-'); \
		printf "\033[2m-> Running %s unit tests (separate module)...\033[0m\n" "$$mod"; \
		modargs="$(submoduleargs)"; \
		if [ -n "$(coverage)" ]; then \
			rm -rf "$(PWD)/tmp/$$tag"; mkdir -p "$(PWD)/tmp/$$tag"; \
			modargs="$$modargs -cover ./... -args -test.gocoverdir=$(PWD)/tmp/$$tag"; \
		else \
			modargs="$$modargs ./..."; \
		fi; \
		echo "(cd $$mod && go test -v $$modargs)"; \
		(cd "$$mod" && go test -v $$modargs) 2>&1 | tee "$(PWD)/test-$$tag.log"; \
		rc=$${PIPESTATUS[0]}; \
...
ifdef coverage
	...
	@for mod in $(SUBMODULES); do \
		tag=$$(echo "$$mod" | tr '/' '-'); \
		go tool covdata textfmt -i="$(PWD)/tmp/$$tag" -o "$(PWD)/tmp/$$tag.cov"; \
	done
endif
test: test-unit
