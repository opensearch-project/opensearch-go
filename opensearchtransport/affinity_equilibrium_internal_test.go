...
// affinityEquilibriumSim runs the discrete-time simulation of the affinity
// scoring feedback loop and returns per-node win counts during the measurement
// phase.
//
...
func affinityEquilibriumSim(nodes []simNode, costFn func(rttBucket float64) float64) {
	const (
	...
	lambda := affinityDecayLambda // ln(2) / 5.0
	...
	for iter := range warmup + measure {
		...
		for i := range nodes {
			counter := max(nodes[i].decay, affinityCounterFloor)
...
// TestAffinityEquilibrium verifies that the affinity scoring feedback loop
...
func TestAffinityEquilibrium(t *testing.T) {
	t.Parallel()

	const (
		measure = 20000 // must match affinityEquilibriumSim
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			affinityEquilibriumSim(nodes, bucketNormCost)
...
// TestAffinityEquilibriumRealisticCost verifies convergence when each node's
...
func TestAffinityEquilibriumRealisticCost(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			affinityEquilibriumSim(nodes, realisticCost)
...
// TestAffinityEquilibriumOldInflation demonstrates that the old inflation
...
func TestAffinityEquilibriumOldInflation(t *testing.T) {
	t.Parallel()
	...
	affinityEquilibriumSim(nodes, oldInflationCost)
