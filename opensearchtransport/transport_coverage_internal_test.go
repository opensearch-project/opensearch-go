...
// Path in place: a retry starts from a copy of the previous attempt's request,
// default MaxRetries is > 0, and a reverse-proxy prefix is a supported address
// shape. Every hop -- retry, Route(), seed fallback, percent-encoded RawPath --
// must start from the caller path so setReqURL prepends once. The cases share
// that fixture; they differ only in which path (wire, Route, EscapedPath) they
// pin.
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(tt.wantRoute) > 0 {
				...
				for _, u := range urls {
					conns = append(conns, &Connection{URL: u, URLString: u.String()})
				}
			...
			if tt.failAfter > 0 {
				require.NotNil(t, c.seedFallbackPool)
			}
...
// recordingRouter hands out conns in order and records the path Route() was
// given, so a test can assert what each attempt was classified on. failAfter
// makes it report ErrNoConnections once that many hops have been served, which
// is how a test cascades to the seed-fallback pool.
