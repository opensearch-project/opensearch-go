...
// client builds a Client against the shared backend. Only the retry and
// timeout fields of cfg matter; the rest are filled in.
func (h *h2Shared) client(cfg Config) *Client {
	return newH2Client(h.t, h.transport, cfg)
}

// get issues a request whose context cannot be cancelled by the test finishing,
// so a caller deadline never confounds what the client itself decides.
func (h *h2Shared) get(tp *Client, path string) (*http.Response, error) {
	h.t.Helper()
...
func (h *h2Shared) getOK(tp *Client, path string) {
	h.t.Helper()
...
func (h *h2Shared) warmup(tp *Client) {
	h.t.Helper()
	h.getOK(tp, "/warm")
...
//
// One Client serves both requests, and the two deadlines are deliberately
