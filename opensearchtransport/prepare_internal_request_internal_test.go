...
func TestPrepareInternalRequest(t *testing.T) {
	...
	tests := []struct {
		name      string
		newTP     func() (*Transport, *recordingSigner)
		path      string
		cancelCtx bool
	...
	}{
		{
			...
			newTP: func() (*Transport, *recordingSigner) {
				return &Transport{userAgent: "ua/1"}, nil
			},
			wantUA:   "ua/1",
			wantHost: "node1:9200",
		},
		...
		},
		{
			...
			newTP: func() (*Transport, *recordingSigner) {
				sig := &recordingSigner{}
				return &Transport{userAgent: "ua/1", signer: sig}, sig
			},
		...
		},
		{
			...
			newTP: func() (*Transport, *recordingSigner) {
				return &Transport{signer: failingSigner(errors.New("sts timeout"))}, nil
			},
		...
		},
		{
			...
			newTP: func() (*Transport, *recordingSigner) {
				return &Transport{username: "admin", password: "secret", userAgent: "ua/1"}, nil
			},
			wantUA:   "ua/1",
			wantHost: "node1:9200",
			wantUser: "admin",
		...
		},
		{
			...
			newTP: func() (*Transport, *recordingSigner) {
				return &Transport{signer: contextAwareSigner{}, userAgent: "ua/1"}, nil
			},
			cancelCtx: true,
...
func TestBackgroundPollersSignRequests(t *testing.T) {
	...
	type poller struct {
		...
		swallows bool
		run      func(t *testing.T, tp *Transport, u *url.URL, conn *Connection) error
	}
	...
	pollers := []poller{
		{
			...
			path:     "/",
			modifier: true,
			run: func(t *testing.T, tp *Transport, u *url.URL, _ *Connection) error {
				t.Helper()
		...
		},
		{
			...
			modifier: true,
			swallows: true,
			run: func(t *testing.T, tp *Transport, _ *url.URL, conn *Connection) error {
				t.Helper()
		...
		},
		{
			name: "nodes info",
			path: "/_nodes/http",
			run: func(t *testing.T, tp *Transport, _ *url.URL, _ *Connection) error {
				t.Helper()
