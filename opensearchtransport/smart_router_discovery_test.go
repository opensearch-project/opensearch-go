...
// TestSmartRouterWithDiscovery verifies the complete flow:
...
// 4. Smart router takes over with discovered nodes
func TestSmartRouterWithDiscovery(t *testing.T) {
	...
	t.Run("Complete seed URL to smart router transition", func(t *testing.T) {
		...
		// Phase 3: After discovery, the smart router should handle requests.
	...
	t.Run("Verify IfEnabledPolicy condition switches after discovery", func(t *testing.T) {
		...
		// and smart router should handle requests.
		...
		bulkBody := "{\"index\":{\"_index\":\"test-smart-router\"}}\n{\"field\":\"value\"}\n"
		...
		t.Cleanup(func() {
			delReq, _ := http.NewRequest(http.MethodDelete, "/test-smart-router", nil)
