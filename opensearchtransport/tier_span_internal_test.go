...
func TestRecordCPUTime_BucketNormalization(t *testing.T) {
	...
	localCounter := local.affinityCounter.load()
	remoteCounter := remote.affinityCounter.load()
...
func TestRecordCPUTime_SameBucketEqualCost(t *testing.T) {
	...
	require.InDelta(t, conn1.affinityCounter.load(), conn2.affinityCounter.load(), 0.01,
...
func TestPolicyChain_SmoothedMaxBucketForIndex(t *testing.T) {
	...
	policy := &IndexAffinityPolicy{cache: cache}
...
func TestFindAffinityCache_WalksTree(t *testing.T) {
	cache := newIndexSlotCache(indexSlotCacheConfig{})
	inner := &IndexAffinityPolicy{cache: cache}
	wrapper := wrapWithAffinity(inner, cache, defaultDecayFactor, &shardCostForReads)

	chain := &PolicyChain{policies: []Policy{wrapper}}

	// findAffinityCache should find the cache through the wrapper.
	found := findAffinityCache(chain)
