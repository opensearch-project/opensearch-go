...
func TestMultiServerPool(t *testing.T) {
	t.Run("Real cluster connection pool", func(t *testing.T) {
		...
		if len(pool.mu.ready) != 2 {
			t.Errorf("Unexpected number of ready connections, want=2, got=%d", len(pool.mu.ready))
		}
	...
	t.Run("Mock server connection pool behavior", func(t *testing.T) {
		...
		// Verify all servers are marked as ready
		pool.mu.Lock()
		readyCount := len(pool.mu.ready)
		deadCount := len(pool.mu.dead)
		pool.mu.Unlock()

		require.Equal(t, 3, readyCount, "Expected 3 ready connections")
