...
func TestNewAffinityCandidate(t *testing.T) {
	t.Parallel()
	...
	t.Run("with RTT data and shard info", func(t *testing.T) {
		...
		info := &shardNodeInfo{Replicas: 3}

		c := newAffinityCandidate(conn, info, &shardCostForReads)
		...
		require.InDelta(t, 5.0, c.DecayCounter, 0.01)
	...
	t.Run("without RTT data", func(t *testing.T) {
		...
		c := newAffinityCandidate(conn, nil, &shardCostForReads)
	...
	t.Run("with primary-only shards", func(t *testing.T) {
		...
		info := &shardNodeInfo{Primaries: 5}
...
func TestBuildAffinityRouteEvent(t *testing.T) {
	t.Parallel()
	...
	event := buildAffinityRouteEvent("test-index", "test-index", 3, 5, candidates, conn1, slot, &shardCostForReads)
...
func TestIndexAffinityPolicyEmitsObserverEvent(t *testing.T) {
	t.Parallel()
...
func TestDocumentAffinityPolicyEmitsObserverEvent(t *testing.T) {
	t.Parallel()
	...
	policy := NewDocumentAffinityPolicy(cache, 0.999)
...
func TestCollectAffinitySnapshot(t *testing.T) {
	t.Parallel()

	t.Run("returns snapshot from IndexAffinityPolicy", func(t *testing.T) {
		t.Parallel()
		policy := NewIndexAffinityPolicy(indexSlotCacheConfig{
			minFanOut:    2,
			maxFanOut:    8,
		...
		snap := collectAffinitySnapshot(policy)
	...
	t.Run("returns nil when no provider exists", func(t *testing.T) {
		t.Parallel()
		// A bare policy without cache doesn't implement affinitySnapshotProvider.
		type barePolicy struct{ Policy }
		snap := collectAffinitySnapshot(&barePolicy{})
		require.Nil(t, snap)
	})
}
...
func TestConnectionInspectionMethods(t *testing.T) {
	...
	t.Run("AffinityLoad", func(t *testing.T) {
		t.Parallel()
		conn := scoreTestConn(t, "node-1", 200*time.Microsecond, 42.5)
		require.InDelta(t, 42.5, conn.AffinityLoad(), 0.01)
	})

	t.Run("AffinityLoad zero", func(t *testing.T) {
		t.Parallel()
		conn := &Connection{URL: &url.URL{Scheme: "https", Host: "node:9200"}}
		require.InDelta(t, 0.0, conn.AffinityLoad(), 0)
	})
}

func TestBuildConnectionMetricAffinityFields(t *testing.T) {
	t.Parallel()

	t.Run("includes RTT and affinity when available", func(t *testing.T) {
		t.Parallel()
		...
		require.NotNil(t, cm.AffinityLoad)
		require.InDelta(t, 10.0, *cm.AffinityLoad, 0.01)
	})
	...
	t.Run("omits affinity load when zero", func(t *testing.T) {
		t.Parallel()
		conn := scoreTestConn(t, "node-1", 512*time.Microsecond, 0)

		cm := buildConnectionMetric(conn)

		require.Nil(t, cm.AffinityLoad)
	})
}
