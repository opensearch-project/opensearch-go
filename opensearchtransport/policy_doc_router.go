...
func (p *DocRouter) Eval(_ context.Context, req *http.Request) (NextHop, error) {
	...
	if len(shardCandidates) > 0 {
		...
		if obs := observerFromAtomic(&p.observer); obs != nil {
			...
			obs.OnRoute(buildRouteEvent(
				...
				routingValue, effectiveRoutingKey, shardNum, true, loadPoolInfoReady(p.config.poolInfoReady), nil, 0,
			))
		}
	...
	if obs := observerFromAtomic(&p.observer); obs != nil {
		...
		obs.OnRoute(buildRouteEvent(
			...
			routingValue, effectiveRoutingKey, shardNum, false, loadPoolInfoReady(p.config.poolInfoReady), nil, 0,
		))
	}
