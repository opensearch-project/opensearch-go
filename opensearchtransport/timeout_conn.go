...
// attemptConns tracks, per underlying net.Conn, which of this transport's
...
// read.
//
// The bookkeeping is per-Transport. Two Transports sharing one *http.Transport
...
func (a *attemptConns) drop(held attemptClaim, markRetiring bool) bool {
	...
	case remaining < 0:
		...
		Debug().Int("users", int(remaining)).
			Msg("attemptConns: connection claim released more than once, not retiring the connection")
		return false
	}
	...
	Debug().Stringer("conn", held.conn.RemoteAddr()).
		Msg("Closed retired HTTP connection")
...
func (a *attemptConns) settle(attemptCtx context.Context, out attemptOutcome) {
	...
	if !a.retire(out.held) {
		...
		Debug().Str("conn", out.connURL).
			Msg("Timed-out HTTP connection not closed by this attempt")
	}
}
