...
//
// Format: bare float (sets preferred and alternate for both read and write
// tables) or comma-separated items in key=value form with optional r:/w: prefix.
//
...
func parseShardCostConfig(spec string) (shardCostMultiplier, shardCostMultiplier, error) {
	...
	for item := range strings.SplitSeq(spec, ",") {
		...
		if !hasEq {
			return reads, writes, &ShardCostConfigError{Key: key, Reason: "missing value"}
		}

		v, parseErr := strconv.ParseFloat(valStr, 64)
		if parseErr != nil {
			return reads, writes, &ShardCostConfigError{Key: key, Reason: "invalid value", Err: parseErr}
		}
