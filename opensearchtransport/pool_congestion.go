...
func (r *poolRegistry) setMaxCwnd(name string, size int32) {
	...
	Debug().
		Str("pool", name).
		Int32("max_cwnd_from", oldMaxCwnd).
		Int32("max_cwnd_to", size).
		Int32("cwnd_from", oldCwnd).
		Int32("cwnd_to", newCwnd).
		Msg("setMaxCwnd: updated congestion window ceiling")
}
...
func (c *clusterSearchAIMD) update(polled []nodeSearchSample) {
	...
	for _, s := range polled {
		...
		newEpoch := clusterNodeSearchEpoch{
			...
			prevWaitTimeNano: epoch.prevWaitTimeNano,
	...
	case congested:
		...
		Debug().
			...
			Int32("cwnd_to", newCwnd).
			Int32("max_cwnd", clusterMaxCwnd).
			Int("nodes", len(polled)).
