...
func TestPolicyChain(t *testing.T) {
	...
	t.Run("OnSuccess delegates to all policy pools", func(t *testing.T) {
		roundRobin := NewRoundRobinPolicy().(*RoundRobinPolicy)
		roundRobin.configurePolicySettings(createTestConfig())

		conn := createTestConnection("http://localhost:9200")
		err := roundRobin.DiscoveryUpdate([]*Connection{conn}, nil, nil)
		require.NoError(t, err)

		// Get zombie conn and promote it
		zombieConn, err := roundRobin.pool.Next()
		require.NoError(t, err)
		roundRobin.pool.OnSuccess(zombieConn)

		chain := NewPolicy(roundRobin).(*PolicyChain)

		// OnSuccess should not panic when delegating
		chain.OnSuccess(conn)
	})

	t.Run("OnFailure delegates to all policy pools", func(t *testing.T) {
		roundRobin := NewRoundRobinPolicy().(*RoundRobinPolicy)
		roundRobin.configurePolicySettings(createTestConfig())
		...
		// OnFailure should not panic, returns nil when pool.OnFailure succeeds
		err = chain.OnFailure(conn)
		require.NoError(t, err)
	})

	t.Run("OnFailure with no matching pools", func(t *testing.T) {
		chain := NewPolicy(NewNullPolicy()).(*PolicyChain)
		conn := createTestConnection("http://localhost:9200")

		// NullPolicy.Eval returns nil pool, so OnFailure has nothing to delegate to
		err := chain.OnFailure(conn)
		require.NoError(t, err)
	})
...
func (p *testPolicyWithError) RotateStandby(_ context.Context, _ int) (int, error) {
	return 0, nil
}
