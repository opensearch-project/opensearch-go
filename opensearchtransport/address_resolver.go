...
// ErrAllResolversFailed is returned when address resolution leaves zero
// usable nodes for a discovery cycle. The built-in resolution handler
// returns it when every AddressResolverFunc invocation returned (nil, error);
// AddressResolverRunner-based resolution returns it when the runner produces
// an empty result set (or only nil-URL entries) with no explicit error.
...
//
// A non-nil URL must have a non-empty Scheme and Host. Malformed URLs
// (e.g. &url.URL{}) are rejected: the default URL is kept and the
// AddressResolverErrors counter is incremented.
//
...
//
// resolve is nil when AddressResolverRunner is configured without a paired
// AddressResolver -- the runner is then expected to compute resolved
// addresses on its own. Runners that delegate to resolve must guard the
// nil case to avoid a panic.
//
// Nodes absent from the returned slice (or present with a nil URL) are
// excluded from the pool. Duplicate Node.IDs in the result are collapsed to
// the first non-nil URL seen for that ID; subsequent entries are dropped
// with a debug log. Returning an empty slice with a nil error causes the
// discovery cycle to fail with ErrAllResolversFailed.
//
