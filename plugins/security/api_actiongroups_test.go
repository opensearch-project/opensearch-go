...
func TestSecurityActiongroupsClient(t *testing.T) {
	...
	testActionGroup := testutil.MustUniqueString(t, "test-actiongroup")
	t.Cleanup(func() {
		client.ActionGroups.Delete(context.Background(), security.ActionGroupsDeleteReq{ActionGroup: testActionGroup})
	})
			...
			Tests: []actiongroupsTests{
				{
					Name: "with request",
					Results: func() (ossectest.Response, error) {
						return client.ActionGroups.Delete(t.Context(), security.ActionGroupsDeleteReq{ActionGroup: testActionGroup})
					},
				},
				{
					Name: "inspect",
							...
							security.ActionGroupsPatchReq{
								Body: security.ActionGroupsPatchBody{
									security.ActionGroupsPatchBodyItem{
										OP:   "add",
										Path: fmt.Sprintf("/%s-patch", testActionGroup),
