...
func TestPatchTenantsReq_GetRequest(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name       string
		req        security.PatchTenantsReq
		wantMethod string
		wantPath   string
		wantErr    bool
	}{
		{
			...
			wantPath:   "/_plugins/_security/api/tenants",
			wantErr:    false,
		},
	}
...
func TestPatchTenants_Roundtrip(t *testing.T) {
	t.Parallel()
	...
	t.Run("success", func(t *testing.T) {
		...
		resp, err := client.PatchTenants(t.Context(), &security.PatchTenantsReq{Body: strings.NewReader("{}")})
		require.NoError(t, err)
	...
	t.Run("error", func(t *testing.T) {
		...
		resp, err := errClient.PatchTenants(t.Context(), &security.PatchTenantsReq{Body: strings.NewReader("{}")})
		require.Error(t, err)
		require.NotNil(t, resp)
	})
}
