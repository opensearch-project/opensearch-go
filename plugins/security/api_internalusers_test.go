...
func TestSecurityInternalUsersClient(t *testing.T) {
	...
	testUser := testutil.MustUniqueString(t, "test-user")
	t.Cleanup(func() {
		client.InternalUsers.Delete(context.Background(), security.InternalUsersDeleteReq{User: testUser})
	})
							...
							security.InternalUsersPatchReq{
								Body: security.InternalUsersPatchBody{
									security.InternalUsersPatchBodyItem{
										OP:   "add",
										Path: fmt.Sprintf("/%s-patch", testUser),
