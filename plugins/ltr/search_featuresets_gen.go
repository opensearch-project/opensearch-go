// SPDX-License-Identifier: Apache-2.0
//
// The OpenSearch Contributors require contributions made to
// this file be licensed under the Apache-2.0 license or a
// compatible open source license.

// Code generated by cmd/osgen; DO NOT EDIT.

package ltr

import (
	"bytes"
	"io"
	"net/http"
	"strconv"

	opensearch "github.com/opensearch-project/opensearch-go/v5"
	"github.com/opensearch-project/opensearch-go/v5/internal/build"
	osparams "github.com/opensearch-project/opensearch-go/v5/internal/params"
	ospath "github.com/opensearch-project/opensearch-go/v5/internal/path"
	"github.com/opensearch-project/opensearch-go/v5/opensearchapi"
)

// SearchFeaturesetsReq represents the request for the ltr.search_featuresets operation.
//
// Search for feature sets in a feature store.
//
// GET /_ltr/_featureset
//
// Available: >= 2.19.0.
type SearchFeaturesetsReq struct {
	// Store specifies the path segment for the request URL.
	Store string

	// Header provides additional HTTP headers for the request.
	Header http.Header

	// Params holds optional query parameters for the request.
	Params *SearchFeaturesetsParams
}

// GetRequest builds the HTTP request from the structured fields.
func (r SearchFeaturesetsReq) GetRequest(method string) (*http.Request, error) {
	path, err := ospath.LTRSearchFeaturesetsPath{
		Store: r.Store,
	}.Build()
	if err != nil {
		return nil, err
	}

	var params map[string]string
	if r.Params != nil {
		params = r.Params.get()
	}

	return build.Request(
		method,
		path,
		nil,
		params,
		r.Header,
	)
}

// SearchFeaturesetsParams represents query parameters for the SearchFeaturesetsReq.
type SearchFeaturesetsParams struct {
	opensearchapi.TimeoutParams
	opensearchapi.DebugParams
	// The offset from the first result (for pagination).
	//
	// Default: 0.
	From int

	// A name prefix to filter feature sets by.
	Prefix string

	// The number of feature sets to return.
	//
	// Default: 20.
	Size int
}

func (r SearchFeaturesetsParams) get() map[string]string {
	var params map[string]string
	set := func(k, v string) {
		if params == nil {
			params = make(map[string]string)
		}
		params[k] = v
	}
	osparams.EncodeTimeout(r.TimeoutParams, set)
	osparams.EncodeDebug(r.DebugParams, set)

	if r.From != 0 {
		set("from", strconv.Itoa(r.From))
	}

	if r.Prefix != "" {
		set("prefix", r.Prefix)
	}

	if r.Size != 0 {
		set("size", strconv.Itoa(r.Size))
	}

	return params
}

// SearchFeaturesetsResp represents the response for the ltr.search_featuresets operation.
//
// Search for feature sets in a feature store.
//
// Available: >= 2.19.0.
type SearchFeaturesetsResp struct {
	opensearchapi.SearchResultJSONValue

	response *opensearch.Response
}

// Inspect returns the raw OpenSearch response for debugging or advanced use.
func (r SearchFeaturesetsResp) Inspect() Inspect {
	return Inspect{Response: r.response}
}

// RawBody returns a fresh reader over the original response bytes,
// useful when the typed response struct is incomplete for your use case.
func (r SearchFeaturesetsResp) RawBody() io.Reader {
	if r.response == nil || len(r.response.RawBody()) == 0 {
		return nil
	}
	return bytes.NewReader(r.response.RawBody())
}
