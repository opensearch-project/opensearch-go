...
func TestManual_DocumentGet(t *testing.T) {
	...
	tests := []struct {
		...
		id      string
		wantErr bool
		check   func(t *testing.T, resp *osapi.GetResp)
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resp, err := client.Get(t.Context(), osapi.GetReq{Index: index, ID: tt.id})
			if tt.wantErr {
				require.Error(t, err)
				return
			}
	...
	t.Run("inspect", func(t *testing.T) {
		...
		res, err := failingClient.Get(t.Context(), osapi.GetReq{Index: index, ID: "doc-1"})
		require.Error(t, err)
...
func TestManual_DocumentMget(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resp, err := client.Mget(t.Context(), osapi.MgetReq{
	...
	t.Run("inspect", func(t *testing.T) {
		...
		res, err := failingClient.Mget(t.Context(), osapi.MgetReq{
...
func TestManual_DocumentUpdate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resp, err := client.Update(t.Context(), osapi.UpdateReq{
				Index:      index,
				ID:         tt.id,
				BodyReader: strings.NewReader(tt.body),
			})
	...
	t.Run("inspect", func(t *testing.T) {
		...
		res, err := failingClient.Update(t.Context(), osapi.UpdateReq{
			Index:      index,
			ID:         "doc-1",
...
func TestManual_DeleteByQuery(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resp, err := client.DeleteByQuery(t.Context(), &osapi.DeleteByQueryReq{
				Index:      []string{index},
	...
	t.Run("inspect", func(t *testing.T) {
		...
		res, err := failingClient.DeleteByQuery(t.Context(), &osapi.DeleteByQueryReq{
			Index:      []string{index},
