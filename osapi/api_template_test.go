...
func TestManual_IndexTemplate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Cleanup(func() {
			_, _ = client.Indices.DeleteIndexTemplate(context.Background(), osapi.IndicesDeleteIndexTemplateReq{Name: tt.template})
		})
	}

	for _, tt := range tests {
		t.Run("create/"+tt.name, func(t *testing.T) {
			resp, err := client.Indices.PutIndexTemplate(t.Context(), osapi.IndicesPutIndexTemplateReq{
				Name:       tt.template,
	...
	for _, tt := range tests {
		t.Run("get/"+tt.name, func(t *testing.T) {
			resp, err := client.Indices.GetIndexTemplate(t.Context(), osapi.IndicesGetIndexTemplateReq{
				Name: tt.template,
			})
			require.NoError(t, err)
	...
	for _, tt := range tests {
		t.Run("exists/"+tt.name, func(t *testing.T) {
			resp, err := client.Indices.ExistsIndexTemplate(t.Context(), osapi.IndicesExistsIndexTemplateReq{
				Name: tt.template,
			})
			require.NoError(t, err)
	...
	for _, tt := range tests {
		t.Run("delete/"+tt.name, func(t *testing.T) {
			resp, err := client.Indices.DeleteIndexTemplate(t.Context(), osapi.IndicesDeleteIndexTemplateReq{
				Name: tt.template,
			})
			require.NoError(t, err)
			require.True(t, resp.Acknowledged)
	...
	t.Run("inspect", func(t *testing.T) {
		...
		res, err := failingClient.Indices.GetIndexTemplate(t.Context(), osapi.IndicesGetIndexTemplateReq{
			Name: "nonexistent",
		})
		require.Error(t, err)
