...
func TestManual_Bulk(t *testing.T) {
	...
	tests := []struct {
		...
		body      string
		wantItems int
		check     func(t *testing.T, resp *osapi.BulkResp)
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resp, err := client.Bulk(t.Context(), osapi.BulkReq{
	...
	t.Run("inspect", func(t *testing.T) {
		...
		res, err := failingClient.Bulk(t.Context(), osapi.BulkReq{
...
func TestManual_Count(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "all documents",
			req:       &osapi.CountReq{Index: []string{index}},
			wantCount: 3,
		},
		{
	...
	t.Run("inspect", func(t *testing.T) {
		...
		res, err := failingClient.Count(t.Context(), &osapi.CountReq{Index: []string{index}})
		require.Error(t, err)
		require.NotNil(t, res)
		osapitest.VerifyInspect(t, res.Inspect())
	})
