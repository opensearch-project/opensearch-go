...
func TestManual_Scroll(t *testing.T) {
	...
	t.Run("scroll lifecycle", func(t *testing.T) {
		searchResp, err := client.Search(t.Context(), &osapi.SearchReq{
			Index:      []string{index},
			BodyReader: strings.NewReader(`{"query":{"match_all":{}},"size":1}`),
			Params:     &osapi.SearchParams{Scroll: 1 * time.Minute},
		})
		require.NoError(t, err)
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				scrollResp, err := client.Scroll.Get(t.Context(), osapi.ScrollReq{
		...
		t.Run("clear scroll", func(t *testing.T) {
			clearResp, err := client.Scroll.Delete(t.Context(), &osapi.ClearScrollReq{
	...
	t.Run("inspect", func(t *testing.T) {
		...
		res, err := failingClient.Scroll.Get(t.Context(), osapi.ScrollReq{
