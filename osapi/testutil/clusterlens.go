...
func clusterLensFSMCheck(client *osapi.Client, expected int) readiness.FSMCheck {
	return func(ctx context.Context, cluster *readiness.Cluster) error {
		...
		clusterJoined := false
		if needsClusterJoin {
			healthReq := &osapi.ClusterHealthReq{
				Params: &osapi.ClusterHealthParams{
					TimeoutParams: osapi.TimeoutParams{Timeout: time.Second},
		...
		catReq := &osapi.CatNodesReq{
			Params: &osapi.CatNodesParams{
				DebugParams: osapi.DebugParams{Format: "json"},
				FullID:      "true",
			},
		}
...
// Reason,Message}Error wrappers that the osapi client emits via
...
// record".
func nodeKey(rec osapi.CatNodesRecord) string {
