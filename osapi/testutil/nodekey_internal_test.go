...
func TestNodeKey(t *testing.T) {
	...
	tests := []struct {
		name string
		rec  osapi.CatNodesRecord
		want string
	}{
		{
			name: "id wins when present",
			rec:  osapi.CatNodesRecord{ID: ptr("g0NX5mUZSdqtKV"), Name: ptr("node-1"), IP: ptr("10.0.0.1")},
			want: "g0NX5mUZSdqtKV",
		},
		{
			name: "name+ip composite when id missing",
			rec:  osapi.CatNodesRecord{Name: ptr("data"), IP: ptr("10.0.0.1")},
			want: "data@10.0.0.1",
		},
		{
			name: "two nodes sharing name disambiguate via ip",
			rec:  osapi.CatNodesRecord{Name: ptr("data"), IP: ptr("10.0.0.2")},
			want: "data@10.0.0.2",
		},
		{
			name: "ip alone when name missing",
			rec:  osapi.CatNodesRecord{IP: ptr("10.0.0.3")},
			want: "10.0.0.3",
		},
		{
			name: "name alone when ip missing",
			rec:  osapi.CatNodesRecord{Name: ptr("orphan")},
			want: "orphan",
		},
		{
...
func TestNodeKey_DistinguishesSameNamedNodes(t *testing.T) {
	...
	ptr := func(s string) *string { return &s }
	a := osapi.CatNodesRecord{Name: ptr("data"), IP: ptr("10.0.0.1")}
	b := osapi.CatNodesRecord{Name: ptr("data"), IP: ptr("10.0.0.2")}
