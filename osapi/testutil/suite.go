...
func (s *OpenSearchTestSuite) EnsureIndex(indexName string) {
	...
	resp, err := s.Client.Indices.Exists(ctx, &osapi.IndicesExistsReq{
	...
	_, err = s.Client.Indices.Create(ctx, osapi.IndicesCreateReq{
		Index: indexName,
	})
...
func (s *OpenSearchTestSuite) CleanupIndex(indexName string) {
	...
	_, err = s.Client.Indices.Delete(ctx, &osapi.IndicesDeleteReq{
		Index: []string{indexName},
	})
