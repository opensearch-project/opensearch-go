...
Package `osapi` provides a strongly-typed Go client for the OpenSearch REST API. It is generated from the official OpenSearch OpenAPI specification by `cmd/osgen`.
...
```go
// Create an index
_, err := client.Indices.Create(ctx, osapi.IndicesCreateReq{
    Index: "products",
    Body:  strings.NewReader(`{"settings":{"number_of_shards":1}}`),
})

// Index a document
_, err = client.Index(ctx, osapi.IndexReq{
    Index:  "products",
    ID:     "1",
...
```go
client.Index(ctx, osapi.IndexReq{Index: "my-index", ...})
```
...
```go
resp, err := client.Indices.Create(ctx, osapi.IndicesCreateReq{Index: "existing"})
if err != nil {
...
Plugin APIs (k-NN, ML, Security, ISM, etc.) live in separate packages under `osapi/plugins/`. They share the same `opensearch.Client` transport but have independent type hierarchies.
