...
If Node B had 50% write utilization: cost = 0.95 + 2.0 \* 0.5^2 = 1.45, score = 0.89. Node C would win instead.
...
The cluster health endpoint requires the `cluster:monitor/health` privilege. The client probes asynchronously and falls back to `GET /` when the privilege is unavailable, with periodic re-probes (default 4h) in case it is granted later. See [Security: Cluster Permissions for Routing and Discovery](config-security.md#cluster-permissions-for-routing-and-discovery) for the complete privilege set.
...
| Availability gain                  | `deltaA = p^2 * eta * (1 - p)`                                                            |
