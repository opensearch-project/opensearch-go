...
The Go client performs active health checks using a two-phase approach: it starts with `GET /` (which requires no special permissions) and asynchronously probes `GET /_cluster/health?local=true` to detect whether richer health data is available. This guide covers how the capability detection works, the permissions required when the OpenSearch Security plugin is enabled, and how to interpret the response.
...
5. **Transient errors** (5xx, network timeout, etc.): The state stays at Pending and the probe is retried on the very next health check cycle. No timestamp is recorded, so transient failures do not lock out retries.

### Runtime Fallback

If the cluster health endpoint was previously working (Available) and later returns 401 or 403 (e.g., the permission was revoked), the client:
...
The formula spreads polls across all clients sharing a server. As the cluster grows, each client polls less frequently to stay within the per-server budget. After each cycle, the interval is recalculated -- if node discovery changes the ready count, the ticker adjusts automatically.
...
- **401/403**: Permission revoked. Reset to Pending and zero out stale data. The connection falls back to `GET /` health checks and re-probes via `MaxRetryClusterHealth`.
- **5xx / network error**: Skip silently; the next cycle retries.
...
The client issues `GET /_cluster/health?local=true` against individual nodes (once capability detection has confirmed it is available -- see above). The `local=true` parameter causes the request to be served from the connected node's local cluster state snapshot rather than requiring a round-trip to the cluster-manager node, making it fast, lightweight, and safe to call even when the cluster-manager is unreachable.
...
The `wait_for_status` parameter (e.g., `?wait_for_status=green&timeout=5s`) blocks the HTTP connection server-side until the cluster reaches the requested status or the timeout expires. This is designed for orchestration scenarios such as waiting for a cluster to become ready after a rolling restart. It is not appropriate for periodic health probes because:

- It holds a connection open for the duration of the wait, wasting both client and server resources on repeated blocked requests.
- It forces a rigid definition of "healthy" at the HTTP layer. A `yellow` cluster is fully functional for most workloads, but `wait_for_status=green` would cause the probe to time out on every single-node cluster.
- The client (or its caller) should decide what cluster status is acceptable, not an HTTP query parameter.

The client uses poll-and-parse instead: issue the request, read the `status` field, and let the caller decide what to do with it.
...
- **401/403 responses**: the node is reachable and responsive, but the client's credentials are wrong or insufficient. The client should **not** mark the connection as dead -- the cluster is fine, the problem is client configuration. These errors should be surfaced clearly so the operator can fix credentials or role mappings.

## Required Permissions

Without the OpenSearch Security plugin installed, all requests are permitted and no authentication is required.

When the Security plugin is enabled, the health check endpoint requires the `cluster:monitor/health` transport action privilege. This is a read-only monitoring action. The `local=true` parameter does not change the permission check -- the same privilege is required regardless.
...
The authentication mechanism used for health checks is the same as for all other requests -- there is nothing special about the health endpoint.
...
If the client is performing health checks and the cluster enables security, health probes will begin returning 401. This is expected. The client should surface this clearly rather than reporting the cluster as unhealthy -- the cluster is reachable, the credentials need to be configured.
...
The `/_cluster/health` endpoint is explicitly exempted from cluster-level blocks. Even when the cluster has a global read-only block or a full block active, this endpoint will still respond. This is intentional -- administrators need to be able to check health to determine _why_ the cluster is blocked.
