...
This guide covers security best practices for using the OpenSearch Go client. Examples use the v4 `opensearchapi` package, which takes request bodies as an `io.Reader`. For `github.com/opensearch-project/opensearch-go/v5/opensearchapi` (typed request bodies, pointer params), see the [v5 security guide](https://github.com/opensearch-project/opensearch-go/blob/v5.0.0/guides/config-security.md).
...
3. **Prefer the slice form over pre-joined strings.** Request fields such as `IndicesDeleteReq.Indices` are `[]string`. Pass one element per intended target and let the client join and encode them. Building a single comma-joined string yourself bypasses the per-element encoding and makes it harder to reason about what each element contains.
...
The `opensearchapi` package takes request bodies as an `io.Reader`. Build them from a Go struct or map with `opensearchutil.NewJSONReader` (or `json.Marshal`) so that every value is escaped. String interpolation into JSON is a security risk because unescaped input can alter the structure of the request.
...
```go
// CORRECT: structured serialization. json.Marshal escapes all values.
...
On the response side, use the typed response fields for structured access. When you need the raw bytes (logging, proxying, custom deserialization), use `RawBody()` on the underlying response:
...
log.Printf("raw response: %s", resp.Inspect().Response.RawBody())
```
...
| Build bodies with `opensearchutil.NewJSONReader` or `json.Marshal` | JSON injection, request structure manipulation      |
