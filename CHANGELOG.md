...
- Fix `opensearchtransport.Client.Perform` silently dropping `io.ReadAll` errors during response buffering via `:=` shadowing; the read error now propagates and `opensearch.Client.Do` handles the `(resp != nil, err != nil)` case under the existing `ErrReadBody` contract ([#859](https://github.com/opensearch-project/opensearch-go/pull/859))
- Fix error-response body not being closed in `opensearch.ParseError` and the v5preview `do()` no-decode path, preventing `http.Transport` from reusing the connection on error responses ([#859](https://github.com/opensearch-project/opensearch-go/pull/859))
