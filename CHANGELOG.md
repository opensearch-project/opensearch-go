...
- Fix background health-check, discovery, and node-stats requests skipping the configured `Signer` (and `Config.Header`). Those pollers built their own `*http.Request` and dispatched it with raw `RoundTrip` after `setReqAuth` only, which never invokes SigV4. Against Amazon OpenSearch Service with IAM-only access every poller returned 403, so resurrection, hardware discovery, AIMD congestion windows, and murmur3 shard-exact routing never engaged. A single `prepareInternalRequest` helper now applies URL rewrite, basic auth, User-Agent, `Config.Header`, `HealthCheckRequestModifier`, and `signRequest` before every raw `RoundTrip`; `signRequest` remains a no-op when `Signer` is nil. `signer/awsv2.SignRequest` now honors the request context so `healthCheckTimeout` bounds credential retrieval ([#1062](https://github.com/opensearch-project/opensearch-go/issues/1062))
