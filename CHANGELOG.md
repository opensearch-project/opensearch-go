...
- `cmd/osapifix`: add the v2 -> v3 hop, so a v2 module can be migrated through to v5. The v2 -> v3 boundary is mostly not mechanizable: the `opensearchapi` package was redesigned from a function-based API into a typed sub-client API, and of 182 exported v2 request types only 16 survive by name. The hop bumps import paths, reports the removed `opensearchapi.*Request` types and the removed root-`opensearch.Client` API methods as manual worklist items, and best-effort rewrites the two seed ops `Ping` and `Indices.Exists` (the call, its raw-response `Status()` handling, and the `Config`/`NewClient` client lifecycle) into compiling v3. Where a transform is not mechanically certain it plants an `_OSAPIFIX_RESOLVE` marker so the build breaks at that spot instead of emitting a wrong value. See `cmd/osapifix/README.md` ([#951](https://github.com/opensearch-project/opensearch-go/pull/951))
