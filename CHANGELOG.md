...
- `cmd/osgen` derives clearer names for `oneOf`/`anyOf` union branches. Aggregation-result branches that the spec titles with terse codes split into readable Go names (`Lterms` -> `LTerms`, `Tdigest` -> `TDigest`, and the full long/string/unsigned/unmapped/significant family), so the `As*`/`New*From*` methods on types like `SearchResultAggregationsValue` read as `AsLTerms`/`AsTDigestPercentiles`. Inline-object branches carry their spec `title` as a semantic name where one exists (e.g. `SearchResultJSONValueSuggestValueItemObject0` -> `...Completion`), with a hyphenated title normalized to PascalCase (`score-ranker-processor` -> `ScoreRankerProcessor`); untitled members keep a positional `ObjectN` name whose index reflects the member's position in the spec array. Branch accessors and constructors no longer repeat the union prefix, so `NewInsightsSourceSourceFromInsightsSourceSourceObject1` becomes `NewInsightsSourceSourceFromObject1` ([#961](https://github.com/opensearch-project/opensearch-go/issues/961))
