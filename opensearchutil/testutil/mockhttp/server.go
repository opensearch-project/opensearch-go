...
func NewServerPool(t *testing.T, name string, count int) (*ServerPool, error) {
	...
	for i := range count {
		...
		mockServer := &Server{
			port: port,
			url:  mustParseURL(GetMockServerURL(port)),
			name: serverName,
		}
...
func (p *ServerPool) StartServer(t *testing.T, index int, handler http.Handler) error {
	...
	t.Logf("Mock server %s started on %s", mockServer.name, addr)
	return nil
}
...
func (p *ServerPool) StopServer(t *testing.T, index int) {
	...
	t.Logf("Mock server %s stopped", mockServer.name)
}
...
// mustParseURL parses a URL and panics if invalid (for internal use)
