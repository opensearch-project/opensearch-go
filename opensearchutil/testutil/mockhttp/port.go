...
func GetMockServerURL(port int) string {
	scheme := DefaultOpenSearchScheme // Default to secure using the constant
	if os.Getenv("SECURE_INTEGRATION") == "false" {
		scheme = "http"
	}
	return scheme + "://" + net.JoinHostPort(MockServerHost, strconv.Itoa(port))
}
