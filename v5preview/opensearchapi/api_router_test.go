...
// TestNewClient_RouterInjection covers the v5preview-specific contract
...
// is not set falsy. v5preview opts every caller into intelligent
...
func TestNewClient_RouterEnvOptOut(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// DiscoverNodesOnStart is one such field: the v5preview
...
// TestNewClient_RouterTruthyEnablesDiscovery confirms the v5preview
...
// verify the discovery side-effect by checking the value the v5preview
