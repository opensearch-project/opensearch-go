...
func TestScriptClient(t *testing.T) {
	...
	scriptID := testutil.MustUniqueString(t, "test-script")
	t.Cleanup(func() {
		client.Script.Delete(context.Background(), opensearchapi.ScriptDeleteReq{ScriptID: scriptID})
	})
	...
	for _, value := range testCases {
		t.Run(value.Name, func(t *testing.T) {
			if strings.Contains(value.Name, "Language") {
				testutil.SkipIfVersion(t, client, "<", "2.4", value.Name)
			}
