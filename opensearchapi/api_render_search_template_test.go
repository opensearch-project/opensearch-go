...
func TestRenderSearchTemplate(t *testing.T) {
	...
	if testutil.IsSecure(t) {
		major, patch, _, err := testutil.GetVersion(t, t.Context(), client)
	...
	testScript := testutil.MustUniqueString(t, "test-search-template")
	t.Cleanup(func() {
		client.Script.Delete(context.Background(), opensearchapi.ScriptDeleteReq{ScriptID: testScript})
	})
