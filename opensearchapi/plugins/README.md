# opensearchapi/plugins

Each plugin is an independent package that wraps a shared `opensearch.Client` to provide strongly-typed access to an OpenSearch plugin's REST API. Plugins are generated from the same OpenAPI specification as the core `opensearchapi` package.
...
import (
    ...
    "github.com/opensearch-project/opensearch-go/v5/opensearchapi/plugins/knn"
)
...
The plugin client and core `opensearchapi.Client` can share the same underlying transport:
...
All import paths are prefixed with `github.com/opensearch-project/opensearch-go/v5/opensearchapi/plugins/`.
