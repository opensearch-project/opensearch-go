...
func TestDataStreamClient(t *testing.T) {
	...
	_, err = client.IndexTemplate.Create(
		...
		opensearchapi.IndexTemplateCreateReq{
			...
			Body: strings.NewReader(fmt.Sprintf(`{"index_patterns":["%s"],"template":{"settings":`+
				`{"index":{"number_of_replicas":"0"}}},"priority":60,"data_stream":{}}`, dataStream)),
		},
	)
	require.NoError(t, err)
	t.Cleanup(func() {
		client.IndexTemplate.Delete(context.Background(), opensearchapi.IndexTemplateDeleteReq{IndexTemplate: dataStream})
	})
