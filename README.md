...
- [Debug logging](USER_GUIDE.md#debugging) - `OPENSEARCH_GO_LOG=debug` prints the client's internal records to stderr with no setup. To route them into the logger your application already runs, install an adapter: [`log-zerolog`](log-zerolog/README.md) is the faster one and its allocation count stays flat however many fields a record carries, while [`log-slog`](log-slog/README.md) adds no dependency, being standard library, at roughly four to five times the cost per record. [Choosing between them](debuglog/README.md#choosing-between-them) has the measured numbers. Nothing is emitted, and nothing allocated, when no logger is installed.
