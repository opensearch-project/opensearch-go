...
- [Debug logging](USER_GUIDE.md#debugging) - `OPENSEARCH_GO_DEBUG=true` prints the client's internal records to stderr with no setup. To route them into the logger your application already runs, install an adapter: [`log-zerolog`](log-zerolog/README.md) is about four times faster per record and allocates a flat 32 bytes however many fields the record carries, while [`log-slog`](log-slog/README.md) adds no dependency, being standard library, and allocates per attribute. Both READMEs carry the measured numbers over identical record shapes. Nothing is emitted, and nothing allocated, when no logger is installed.
