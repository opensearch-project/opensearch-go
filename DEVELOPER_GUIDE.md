...
The `opensearchapi/` package on this branch is hand-written. The generated API surface ships as `github.com/opensearch-project/opensearch-go/v5/opensearchapi` and is developed on the `main` branch. `osgen api` has no output target on v4: its defaults still name the removed `v5preview/` directory, and `make gen` does not run it. See `UPGRADING.md` for migration guidance.
...
The `x-error-responses` extension on a spec operation declares the categories of partial failure that operation can produce. For each category named on an operation, `osgen api` emits the following into the package it generates (on `main`, the v5 `opensearchapi` package):
...
To regenerate the `internal/path` builders (downloads the spec automatically if not cached):
...
By default, `make gen` includes path builders for all operations across every OpenSearch version. Three Makefile variables scope the generated output to a version window:
...
```
# Path builders for OpenSearch 2.x+ only, dropping deprecated APIs
make gen GEN_MIN_VERSION=2.0 GEN_REMOVE_DEPRECATED=2.0

# Path builders for a specific version range
