...
func TestParseVerifyDeadAfter(t *testing.T) {
	tests := []struct {
		...
		{"bool true uses default", "true", VerifyDeadAfterDefault, nil},
		{"bool 1 uses default", "1", VerifyDeadAfterDefault, nil},
		...
		{"unparseable errors", "notaduration", 0, ErrInvalidVerifyDeadAfter},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotDur, err := ParseVerifyDeadAfter(tt.val)
