...
// together under mu. Under `go test` (testing.Testing() true) it panics so the
// defect surfaces loudly; in a production binary it emits a debug diagnostic
// through the installed logger (if any) and lets sweep reconcile, rather than
// crashing a live process over a self-healing inconsistency. Caller holds mu.
