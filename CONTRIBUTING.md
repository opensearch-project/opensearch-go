...
You may type this line on your own when writing your commit messages. However, if your git config `user.name` and `user.email` are set properly, you can use `-s` or `--signoff` to add the `Signed-off-by` line to the end of the commit message.
