...
//     references such a field, that is an osapifix bug - the field table is
//     incomplete - and the rewrite fails loudly rather than silently dropping a
//     value. See the engine's unclassified handling.
...
// unqualified name within a package pair. The engine resolves the source struct
...
// FieldChange; the engine fails loudly if the consumer references it, so the
...
// engine fails loudly if the consumer references it. This is deliberate:
...
func dispositionChange(
	...
	if !ok {
		...
		// guess; the engine turns this into a loud error if the field is used.
		return FieldChange{
