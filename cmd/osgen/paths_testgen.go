...
// testTmpl is the test-generation template, parsed once at startup.
//
...
const testTmplSrc = `// SPDX-License-Identifier: Apache-2.0
//
// The OpenSearch Contributors require contributions made to
// this file be licensed under the Apache-2.0 license or a
// compatible open source license.
...
package {{.Pkg}} //nolint:testpackage // see file header
...
func generateTests(builders []builder, pkg string) (string, error) {
	testBuilders := make([]testBuilder, 0, len(builders))
	...
	for _, op := range b.Ops {
		...
		case opCase:
			if len(stack) > 0 && stack[len(stack)-1].isSwitch {
				...
				case top.taken:
					top.active = false
				case condsAllMatch(op.Conditions):
					top.active = true
					top.taken = true
				default:
					top.active = false
