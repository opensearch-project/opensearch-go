...
// opensearchapi.*Request TYPES (idiom 1) are surfaced by the linter's
...
// are caught by the linter's removed-type diagnostic (idiom 1), and the shape
...
var hopV2toV3 = hop{
	...
	// way the linter can act on; the whole function-based API surface is gone and
	...
	SemanticFollowups: []string{
		...
		"Idiom 2 (root client): the root opensearch.Client no longer exposes API methods (only Transport survives). " +
			...
			"client.<Endpoint>(ctx, &<Endpoint>Req{...}). osapilint now rewrites the seed ops Ping and Indices.Exists " +
			"best-effort (the call, its raw-response Status/Warnings handling, and the Config client lifecycle); " +
			"review each rewrite and resolve any _OSAPILINT_RESOLVE marker. The remaining root-client endpoints are not " +
