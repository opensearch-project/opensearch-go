...
// source->target chain (see compose.go); the linter itself is version-agnostic.
...
// down the walk so no linter function reaches for a package global.
...
// not a rename), so the linter reports it as a MANUAL worklist item rather than
