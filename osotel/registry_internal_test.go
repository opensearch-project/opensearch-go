...
func TestWithLogger(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mp, _ := newTestMeter(t)
			reg, err := NewWithOptions(mp.Meter("test"), nil, []Option{WithLogger(tt.logger(explicit))})
			require.NoError(t, err)
			...
			require.NotContains(t, global.messages(), "osotel registry running")
			require.NotContains(t, global.messages(), "osotel registry stopped")
		})
	}
}
...
func TestDefaultLoggerResolvesPerMessage(t *testing.T) {
	...
	mp, _ := newTestMeter(t)
	reg, err := NewWithOptions(mp.Meter("test"), nil, nil) // no WithLogger: takes the default
	require.NoError(t, err)
	...
	require.Eventually(t, func() bool {
		return slices.Contains(first.messages(), "osotel registry running")
	...
	require.Contains(t, second.messages(), "osotel registry stopped",
		"shutdown message did not follow the swapped global")
	require.NotContains(t, first.messages(), "osotel registry stopped",
